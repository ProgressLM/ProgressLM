...
# GPT-5 Text-Based Progress Evaluation Processor Runner Script
# 使用方法: ./run_text_demo_processor.sh
...
# 拓展功能默认配置
DEFAULT_LIMIT="6"        # 默认不限制数量，如需限制设置为数字，例如: DEFAULT_LIMIT=100
...
print_banner() {
    echo -e "${CYAN}"
    echo "============================================================"
    echo "     GPT-5 Text-Based Progress Evaluation Processor        "
    echo "============================================================"
    echo -e "${NC}"
}
...
print_help() {
    ...
    echo '  {'
    echo '    "id": "sample_id",'
    echo '    "task_goal": "inserting a battery...",'
    echo '    "text_demo": ["step1", "step2", "step3"],'
    echo '    "total_steps": 3,'
    echo '    "stage_to_estimate": "image.jpg",'
    echo '    "closest_idx": 1,'
    echo '    "progress_score": "33%",'
    echo '    "data_source": "source_name"'
    echo '  }'
}
...
check_requirements() {
    ...
    if [ ! -f "$PROCESSOR_SCRIPT" ]; then
        ...
        echo "请确保text_demo_processor.py脚本存在"
        exit 1
    fi
...
try:
    ...
    required_fields = ['id', 'task_goal', 'text_demo', 'stage_to_estimate', 'closest_idx', 'progress_score']
    missing = [f for f in required_fields if f not in data]
    if missing:
        print(f'缺少必需字段: {missing}')
        sys.exit(1)
    if not isinstance(data['text_demo'], list):
        print('text_demo字段必须是列表')
        sys.exit(1)
    print('数据格式正确')
...
" 2>&1; then
            ...
            echo -e "${RED}❌ 数据格式验证失败${NC}"
            echo "请确保数据包含所有必需字段且格式正确"
            exit 1
        fi
    fi
...
show_results_summary() {
    if [ -f "$OUTPUT_FILE" ]; then
        ...
        # 统计成功和失败
        SUCCESS_COUNT=$(grep -c '"status": "success"' "$OUTPUT_FILE" 2>/dev/null || echo 0)
        ERROR_COUNT=$(grep -c '"status": "error"' "$OUTPUT_FILE" 2>/dev/null || echo 0)
...
with open('$OUTPUT_FILE', 'r') as f:
    for line in f:
        try:
            ...
            if data.get('meta_data', {}).get('status') == 'success':
                total_count += 1
                if data.get('score') == data.get('ground_truth_score'):
                    match_count += 1
        except:
            pass
if total_count > 0:
    print(f'{match_count}/{total_count} ({100*match_count/total_count:.1f}%)')
else:
    print('0/0')
" 2>/dev/null || echo "无法计算")
            echo -e "  预测与真实值匹配: ${CYAN}$MATCH_COUNT${NC}"
        fi
    ...
    for line in f:
        try:
            ...
            if data.get('meta_data', {}).get('status') == 'success':
                if data.get('score'):
                    try:
                        score_val = int(data['score'].replace('%', ''))
                        scores.append(score_val)
                    except:
                        pass
                if data.get('ref'):
                    try:
                        refs.append(int(data['ref']))
                    except:
                        pass
                if data['meta_data'].get('tokens_used'):
                    tokens.append(data['meta_data']['tokens_used'])
        except:
            pass
...
if refs:
    ...
    for i in range(1, max(refs) + 1):
        count = refs.count(i)
        if count > 0:
            print(f'  步骤 {i}: {count} 次 ({100*count/len(refs):.1f}%)')

if tokens:
    print(f'\nToken使用:')
    print(f'  平均: {statistics.mean(tokens):.0f}')
    print(f'  总计: {sum(tokens):,}')
    print(f'  预估成本: \${sum(tokens) * 2.25 / 1_000_000:.4f} (简化计算)')
" 2>/dev/null || echo "无法生成详细分析"
    fi
}
...
echo -e "${BLUE}配置信息:${NC}"
echo "================================"
echo "处理器类型: ${CYAN}Text-Based Demo${NC}"
...
# 创建日志目录（如果需要）
LOG_DIR="$(dirname "$OUTPUT_FILE")/logs"
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/processing_$(date +%Y%m%d_%H%M%S).log"

# 运行处理器
echo -e "${CYAN}日志文件: $LOG_FILE${NC}"
run_processor 2>&1 | tee "$LOG_FILE"
RESULT=${PIPESTATUS[0]}
...
echo ""
echo "  # 仅查看成功的结果"
echo "  grep '\"status\": \"success\"' $OUTPUT_FILE | jq '.'"
echo ""
echo "  # 查看日志"
echo "  less $LOG_FILE"
...
if [ -f "$OUTPUT_FILE" ]; then
    ...
    if [ $ERROR_COUNT -gt 0 ]; then
        echo ""
        echo -e "${YELLOW}提示: 有 $ERROR_COUNT 个失败的样本${NC}"
        echo "您可以使用以下命令重新处理失败的样本:"
        echo "  $0 -k \"$API_KEY\" -i \"$INPUT_FILE\" -o \"$OUTPUT_FILE\" -d \"$IMAGE_DIR\" -r"
    fi
fi
